using Entropies: ProbabilitiesEstimator, Entropy, IndirectEntropy, Shannon
...
"""
    conditional_mutualinfo(e::IndirectEntropy, x, y, z)

Estimate ``I(x; y | z)``, the  conditional mutual information between `x` and `y` given
`z`, by a sum of marginal entropies (whose type is dictated by `e`), using the
provided [`IndirectEntropy`](@ref) estimator.
"""
function conditional_mutualinfo(e::IndirectEntropy,
...
to direct estimation using [`ProbabilityEstimator`](@ref)s or [`IndirectEntropy`](@ref)
estimators.

See also [`MutualInformation`](@ref) estimators [`Kraskov1`](@ref), [`Kraskov2`](@ref),
and [`ConditionalMutualInformation`](@ref) estimators.
"""
function conditional_mutualinfo(est::MutualInformation,
        x::Vector_or_Dataset, y::Vector_or_Dataset, z::Vector_or_Dataset)
    return mutualinfo(est, x, Dataset(y, z)) - mutualinfo(est, x, z)
end

function conditional_mutualinfo(est::ConditionalMutualInformation,
        x::Vector_or_Dataset, y::Vector_or_Dataset, z::Vector_or_Dataset)
    return mutualinfo(est, x, Dataset(y, z)) - mutualinfo(est, x, z)
end
