...
"""
    ExtendedPalusLimit(ext_finer = 1, ext_coarser = 0, min_subdivs = 2, max_subdivs = 8)

The [`PalusLimit`](@ref) heuristic partitions the state space by splitting each marginal into at 
most ``N^{\\frac{1}{D+1}}`` equally-spaced intervals, where ``N`` is the number of observations 
...
(e.g. `ext_finer = 2` adds one more finer partition), or coarser
...
function estimate_partition(x::AbstractVector{T}, dim::Int, heuristic::BinningHeuristic) where T
    estimate_partition(length(x), dim, heuristic)
end
